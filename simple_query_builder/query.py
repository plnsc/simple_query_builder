...
class Query(_Dialect):
    ...
    def set_columns(self, columns: list[str]) -> "Query":
        self._columns = [self.wrapper("identifier", column) for column in columns]
        return self
    ...
    def dump_update(self) -> str:
        update_symbol, set_symbol, assign_symbol, where_symbol = self.statement(
            "update"
        )
        ...
        if self._filter and len(self._filter._parts):
            result.append(
                self.separator("empty", [where_symbol, self._filter.dump_parts()])
            )
    ...
    def dump_delete(self) -> str:
        ...
        result = [delete_symbol, self.wrapper("identifier", self._entity)]
